...
def get_tiny_images(img_paths):
    ...
    for img_path in tqdm(img_paths):
        ...
        # resize
        resized_dim = 16
        img = cv2.resize(img, (resized_dim, resized_dim), interpolation=cv2.INTER_AREA)
        # flatten and normalize
        tiny_img_feat = img.flatten().astype(np.float32)
        tiny_img_feat = tiny_img_feat - np.mean(tiny_img_feat)
        tiny_img_feat = tiny_img_feat / np.linalg.norm(tiny_img_feat)
        tiny_img_feats.append(tiny_img_feat)
...
###### Step 1-b-1
def build_vocabulary(img_paths, vocab_size=4000):
    '''
    Input : 
    ...
    for img_path in img_paths:
        img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
        _, descriptors = dsift(img, step=[step_num, step_num], fast=True)
...
def get_bags_of_sifts(img_paths, vocab):
    ...
    for img_path in img_paths:
        ...
        histogram, _ = np.histogram(assignments, bins=len(vocab))
        histogram_norm = [(float(i)-np.mean(histogram))/sum(histogram) for i in histogram]
...
def nearest_neighbor_classify(train_img_feats, train_labels, test_img_feats):
    ...
    k = 6
    nearest_k_indices = np.argsort(dists, axis=1)[:, :k]

    for indices in nearest_k_indices:
        label_ids = [CAT2ID[train_labels[idx]] for idx in indices]
        final_id = max(set(label_ids), key=label_ids.count)
        test_predicts.append(CAT[final_id])
