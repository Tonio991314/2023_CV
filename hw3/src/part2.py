        ...
        if ret:  ## check whethere the frame is legal, i.e., there still exists a frame
            ...
            if ids is not None:
                ...
                if 0 in ids:
                    ...
                    proj_corns = np.array([corners[idx][0][0], corners[idx][0][1], corners[idx][0][2], corners[idx][0][3]])
                    H = solve_homography(ref_corns, proj_corns)
            # TODO: 3.apply backward warp
            # function call to warping()
                    xmin = np.min(corners[0][0][:,0]).astype(np.int64)
                    xmax = np.max(corners[0][0][:,0]).astype(np.int64)
                    ymin = np.min(corners[0][0][:,1]).astype(np.int64)
                    ymax = np.max(corners[0][0][:,1]).astype(np.int64)
